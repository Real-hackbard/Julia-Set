...
uses
   ...
   Vcl.ComCtrls, Vcl.Shell.ShellCtrls, Vcl.Clipbrd, Vcl.Printers,
   Vcl.ExtCtrls, Vcl.StdCtrls, Vcl.Grids, Vcl.Buttons, Vcl.Menus;
...
procedure TForm1.juliaerzeugen(var bitmap:tbitmap;Sender: TObject);
var bit:tbitmap;
    birect:trect;
begin
    bit:=tbitmap.create;
...
procedure TForm1.pb_mandelPaint(Sender: TObject);
...
procedure TForm1.pb_mandelMouseDown(Sender: TObject;
...
procedure TForm1.pb_mandelMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
...
procedure TForm1.pb_mandelMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var breite,hoehe:Integer;
begin
    breite:=pb_mandel.width;
...
procedure TForm1.S_AnimationClick(Sender: TObject);
begin
    timer1.enabled:=not timer1.enabled;
    if timer1.enabled then button2.caption:='Cancel'
...
begin
    ...
    case sel of
      0 : begin
            ...
            if (mandelpunkt[auswahlpunkt].x>1.5) or (mandelpunkt[auswahlpunkt].x<-2.5) then
              juauf:=-juauf;
      ...
      1 : begin
            ...
            if (mandelpunkt[auswahlpunkt].y>2) or (mandelpunkt[auswahlpunkt].y<-2) then
              juauf:=-juauf;
...
begin
  ...
  begin
    try
      ...
      Bitmap.Width:=julia1.Width;
      Bitmap.Height:=julia1.Height;
      Bitmap.Canvas.CopyRect(Bounds(0,0,bitmap.Width, Bitmap.Height),
        julia1.Canvas, julia1.ClientRect);
...
begin
  ...
  begin
    try
      Bitmap:=TBitmap.Create;
      Bitmap.PixelFormat := pf24bit;
      Bitmap.Width:=julia2.Width;
      Bitmap.Height:=julia2.Height;
      Bitmap.Canvas.CopyRect(Bounds(0,0,bitmap.Width, Bitmap.Height),
        julia2.Canvas, julia2.ClientRect);
...
begin
  ...
  begin
    try
      Bitmap:=TBitmap.Create;
      Bitmap.PixelFormat := pf24bit;
      Bitmap.Width:=julia3.Width;
      Bitmap.Height:=julia3.Height;
      Bitmap.Canvas.CopyRect(Bounds(0,0,bitmap.Width, Bitmap.Height),
        julia3.Canvas, julia3.ClientRect);
...
begin
  ...
  begin
    try
      Bitmap:=TBitmap.Create;
      Bitmap.PixelFormat := pf24bit;
      Bitmap.Width:=julia4.Width;
      Bitmap.Height:=julia4.Height;
      Bitmap.Canvas.CopyRect(Bounds(0,0,bitmap.Width, Bitmap.Height),
        julia4.Canvas, julia4.ClientRect);
...
procedure TForm1.Save5Click(Sender: TObject);
var
  Bitmap: TBitmap;
begin
  if SaveDialog1.Execute then
  begin
    try
      Bitmap:=TBitmap.Create;
      Bitmap.PixelFormat := pf24bit;
      Bitmap.Width:=julia5.Width;
      Bitmap.Height:=julia5.Height;
      Bitmap.Canvas.CopyRect(Bounds(0,0,bitmap.Width, Bitmap.Height),
        julia5.Canvas, julia5.ClientRect);
...
procedure TForm1.Save6Click(Sender: TObject);
var
  Bitmap: TBitmap;
begin
  if SaveDialog1.Execute then
  begin
    try
      Bitmap:=TBitmap.Create;
      Bitmap.PixelFormat := pf24bit;
      Bitmap.Width:=julia6.Width;
      Bitmap.Height:=julia6.Height;
      Bitmap.Canvas.CopyRect(Bounds(0,0,bitmap.Width, Bitmap.Height),
        julia6.Canvas, julia6.ClientRect);
...
begin
    ...
    if timer2.enabled then button3.caption:='Stop'
                      else button3.caption:='Color Animation';
end;
